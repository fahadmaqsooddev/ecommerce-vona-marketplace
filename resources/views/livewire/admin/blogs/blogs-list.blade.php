<div>
    <div class="row">
        <div class="col-12">
            <div class="card">
                {{-- Header --}}
                <div class="p-3">
                    {{-- Title --}}
                    <div class="mb-2">
                        <h5 class="mb-0 fw-bold">Blogs</h5>
                    </div>

                    {{-- Search + Button --}}
                    <div class="d-flex align-items-center gap-2">

                        {{-- Search --}}
                        <div class="input-group" style="max-width: 350px;">
                            <span class="input-group-text bg-white border-end-0">
                                <i class="fas fa-search text-muted"></i>
                            </span>
                            <input type="text"
                                   wire:model.live.debounce.400ms="search"
                                   class="form-control border-start-0"
                                   placeholder="Search...">
                            @if($search)
                                <button class="btn btn-outline-secondary"
                                        wire:click="$set('search', '')">
                                    <i class="fas fa-times"></i>
                                </button>
                            @endif
                        </div>

                        {{-- Spacer --}}
                        <div class="flex-grow-1"></div>

                        {{-- Add Button --}}
                        <button class="btn btn-success flex-shrink-0"
                                @click="$dispatch('open-modal')">
                            <i class="fas fa-plus"></i>
                            <span class="d-none d-sm-inline"> Add Blog</span>
                        </button>

                    </div>
                </div>

                
                <div wire:loading class="text-center py-4">
                    <span class="spinner-border text-primary"></span>
                    <p class="text-muted mt-2">Please wait...</p>
                </div>

                <div wire:loading.remove>
                    <div class="row m-2 g-3">
                        @if($blogs && $blogs->count())
                            @foreach($blogs as $blog)
                                <div class="col-12 col-sm-6 col-lg-4">
                                    <div class="card h-100 shadow-sm border-0" style="border-radius: 12px; overflow: hidden;">

                                     
                                        <div style="position: relative;">
                                            <img src="{{ $blog->image_url }}"
                                                 alt="{{ $blog->heading }}"
                                                 style="width:100%; height:180px; object-fit:cover;">
                                        </div>

                                    
                                        <div class="card-body d-flex flex-column p-3">
                                            <h6 class="font-weight-bold mb-1">{{ $blog->heading }}</h6>
                                            <p class="text-muted small flex-grow-1 mb-0">{{ $blog->description }}</p>
                                        </div>

                                        
                                        <div class="d-flex px-3 pb-3" style="gap: 10px;">
                                            
                                            <button class="btn btn-warning btn-sm flex-fill"
                                                    style="border-radius: 8px;"
                                                    @click="$dispatch('open-edit-modal', { id: {{ $blog->id }} }); $wire.dispatchTo('admin.blogs.edit-blog', 'load-blog', { id: {{ $blog->id }} })">
                                                <i class="fas fa-edit mr-1"></i> Edit
                                            </button>

                                            
                                            <button class="btn btn-danger btn-sm flex-fill"
                                                    style="border-radius: 8px;"
                                                    wire:click="delete({{ $blog->id }})"
                                                    wire:confirm="Are you sure you want to delete this blog?"
                                                    wire:loading.attr="disabled"
                                                    wire:target="delete({{ $blog->id }})">
                                                <span wire:loading.remove wire:target="delete({{ $blog->id }})">
                                                    <i class="fas fa-trash mr-1"></i> Delete
                                                </span>
                                                <span wire:loading wire:target="delete({{ $blog->id }})">
                                                    <i class="fas fa-spinner fa-spin"></i> Deleting...
                                                </span>
                                            </button>
                                        </div>

                                    </div>
                                </div>
                            @endforeach
                        @else
                            <div class="text-center py-5 w-100">
                                <i class="fas fa-folder-open fa-3x text-muted"></i>
                                <p class="text-muted mt-3">
                                    @if($search)
                                        No results for "<strong>{{ $search }}</strong>"
                                    @else
                                        No blogs found
                                    @endif
                                </p>
                            </div>
                        @endif
                    </div>

                    {{-- Pagination --}}
                    @if($blogs && $blogs->hasPages())
                        <div class="d-flex flex-wrap justify-content-between align-items-center px-3 mt-3 mb-3 gap-2">
                            <small class="text-muted">
                                Showing {{ $blogs->firstItem() }} to {{ $blogs->lastItem() }}
                                of {{ $blogs->total() }} results
                            </small>
                            {{ $blogs->links('pagination::bootstrap-4') }}
                        </div>
                    @endif
                </div>

            </div>
        </div>
    </div>

    @livewire('admin.blogs.add-blog')
    @livewire('admin.blogs.edit-blog')
</div>